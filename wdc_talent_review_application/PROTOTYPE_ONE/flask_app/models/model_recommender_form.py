...
# *************************************Converting a List to String*********************************************************************************
        ...
        query = "INSERT INTO recommender_forms ( user_id, nominee_id, work_contributions) VALUES ( %(user_id)s, %(nominee_id)s, %(work_contributions)s );"
        # store the returned info using a Variable
        results = connectToMySQL(DATABASE).query_db(query, data)
        # retuns an id so make sure to user get method to retrieve the actual data for the object / instance
        return results
...
############################################# READ (DATA type Returned is list of dictionaries) #############################################
    ...
    def get_all_recommendations(cls): # Takes in 1 parameter
        ...
        query = "SELECT * FROM recommender_forms;"
...
############################################# READ (DATA type Returned is list of dictionaries) #############################################
    ...
    def get_one_recommender(cls, data):
        ...
        query = "SELECT * FROM recommender_forms WHERE id = %(id)s;"
